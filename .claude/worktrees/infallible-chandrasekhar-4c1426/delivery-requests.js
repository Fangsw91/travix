...
function renderRequestCard(shipment) {
    ...
    const reward = (parseFloat(shipment.price || 0) * 0.90).toFixed(2);
    ...
    return `
        <div class="request-card">
            <div class="request-card-header">
                ...
                <span class="category-badge">${shipment.category}</span>
            </div>
            <p class="request-route">📍 ${shipment.pickup_location} → ${shipment.destination}</p>
            <p class="request-date">📅 ${date} · ${shipment.weight} kg</p>
            <div class="request-card-footer">
                <strong class="request-reward">You earn: $${reward}</strong>
                <button class="btn-accept"
                    onclick="openAcceptModal(${shipment.id}, '${shipment.item_name.replace(/'/g, "\\'")}', '${shipment.weight}', '${date}', '$${reward}')">
                    Accept
                </button>
            </div>
        </div>
...
async function loadDeliveryRequests() {
    const container = document.querySelector('.requests-grid') || document.querySelector('.requests-list');
    if (!container) return;
