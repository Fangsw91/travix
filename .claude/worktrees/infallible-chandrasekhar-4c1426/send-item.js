...
function submitForm(formData, submitBtn, originalText) {
    ...
    const price = (parseFloat(formData.weight) || 0) * 15;
    ...
    ShipmentAPI.create({
        ...
        pickup_country: formData.pickup,
        ...
        price: price,
        special_instructions: formData.deliveryAddress || ''
    }).then(data => {
        localStorage.setItem('shipmentId', data.shipment?.id || '');
        ...
        showError(firstError || err.message || 'Failed to submit request. Please try again.');
    });
}
